<?php 
  ...
  if(isset($_POST['payout'])){
    if($_POST['name'] != '' && $_POST['mail'] && $_POST['number'] != '' && $_POST['date'] != '' && $_POST['crypto'] != ''){
      if($_POST['date'] < date('d-m-Y')){
        $msg = "La date d'expiration est incorrect";
      }
      else{
        $_SESSION['user']->checkout();
          ...
          <article class="milieugauchepay">
            ...
            <div id="backgauchepay">
              ...
              <form method="post">
                ...
                <input id="inputpro2" type="text" name="name" required><br>
                ...
                <input id="inputpro2" type="text" name="number" required><br>

                <label class="labelpro" for="confirmation">Date d'expiration :</label>
                <input type="date" id="inputpro2" name="date"
                value="2010-01-01"
                min="<?php echo date('Y-m-d'); ?>" max="2040-12-31" required><br>
              
                <label class="labelpro" for="confirmation">CVV :</label>
                <input id="inputpro2" maxlength="3" type="text" name="crypto" required><br>
                <input type="submit" class="pay" name="payout" value="Payer">
                <?php if($msg != ''){ echo $msg; } ?>
              </form>
